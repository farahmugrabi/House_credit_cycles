...
(sd(data$GNIq_qoq)-sd(data$GNI_cl_qoq))*100 #dispertion of linear interpolation is x percentage points above the one obtained with Chow-Li

# ggsave(paste0(save_plots,"/GNI.png"), GNI_plot, dpi = 500, height = 20, width = 25)
# ggsave(paste0(save_plots,"/GNI.pdf"), GNI_plot, height = 20, width = 25)
# ggsave(paste0(save_plots,"/GNI_yoy.png"), GNI_yoy_plot, dpi = 500, height = 20, width = 25)
# ggsave(paste0(save_plots,"/GNI_yoy.pdf"), GNI_yoy_plot, height = 20, width = 25)
# ggsave(paste0(save_plots,"/GNI_r.pdf"), GNI_r_plot, height = 20, width = 25)

write.csv(data, paste0(path, "/data_GNI.csv"), row.names = FALSE)
...
data<- data %>%
  ...
  mutate(Nat_cred_l_sa= ifelse(!is.na(Nat_cred), (log10(as.vector(x13(ts(Nat_cred, frequency = 4))$final[[1]][,"sa"]))),NA)) %>% #corrected, national credit already expressed in billion
  mutate(Nat_cred_sa= ifelse(!is.na(Nat_cred), as.vector(x13(ts(Nat_cred, frequency = 4))$final[[1]][,"sa"]),NA)) %>% #corrected
...
nfc_plot<- ggplot(data=data, aes(x=Date))+
  ...
  theme(legend.position = "bottom",
        ...
        axis.title=element_text(size=25),
        legend.text =element_text(size=25))
# ggsave(paste0(save_plots,"/nfc_credit.png"), nfc_plot, dpi = 500, height = 20, width = 25)
# ggsave(paste0(save_plots,"/nfc_credit.pdf"), nfc_plot, height = 20, width = 25)
...
cred_plot<- ggplot(data=data, aes(x=Date))+
  ...
  theme(legend.position = "bottom",
        ...
        axis.title=element_text(size=25),
        legend.text =element_text(size=25))
# ggsave(paste0(save_plots,"/cred_plot.png"), cred_plot, dpi = 500, height = 20, width = 25)
# ggsave(paste0(save_plots,"/cred_plot.pdf"), cred_plot, height = 20, width = 25)
...
hp_plot<- ggplot(data=data, aes(x=Date))+
  ...
  theme(legend.position = "bottom",
        ...
        axis.title=element_text(size=25),
        legend.text =element_text(size=25))
# ggsave(paste0(save_plots,"/hp_plot.png"), hp_plot, dpi = 500, height = 20, width = 25)
# ggsave(paste0(save_plots,"/hp_plot.pdf"), hp_plot, height = 20, width = 25)
...
gni_plot<- ggplot(data=data, aes(x=Date))+
  ...
  theme(legend.position = "bottom",
        ...
        axis.title=element_text(size=25),
        legend.text =element_text(size=25))
# ggsave(paste0(save_plots,"/gni_plot.png"), gni_plot, dpi = 500, height = 20, width = 25)
# ggsave(paste0(save_plots,"/gni_plot.pdf"), gni_plot, height = 20, width = 25)

#Export data
write.csv(data_model, paste0(path, "/data_model.csv"), row.names = FALSE)
write.csv(data, paste0(path, "/data_full.csv"), row.names = FALSE)
write.csv(data_model_r, paste0(path, "/data_model_r.csv"), row.names = FALSE)
...
write.xlsx(stats_table, file = "descriptive_statistics.xlsx", row.names = FALSE)
