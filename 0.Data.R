...
# gdp = read.csv("https://data-api.ecb.europa.eu/service/data/MNA/Q.N.IE.W2.S1.S1.B.B1GQ._Z._Z._Z.EUR.V.N?format=csvdata") %>% 
#   bind_rows(.) %>% 
#   dplyr::select(REF_AREA, TIME_PERIOD, OBS_VALUE) %>% 
#   rename(ISO2 = REF_AREA, Date = TIME_PERIOD, GDP = OBS_VALUE) %>% 
#   mutate(Date = as.Date(as.yearqtr(Date, format = "%Y-Q%q"))) %>% 
#   # mutate(GDP_y= rollapply(data = GDP, width = 4, FUN =sum/1000,align = "right", fill = NA,na.rm = TRUE))
...
data_standardg<- data %>% 
  ...
  mutate(credit_to_gdp= Total_credit/GDP_y )
